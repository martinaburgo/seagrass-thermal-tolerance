...
### Fit the model 
```{r}
seagrass.form <- bf(Survival ~ scale(log(Time))*scale(difference_population) + scale(av_population) + scale(mtwa_population) + (1|Study),
                   family = zero_one_inflated_beta())

get_prior(seagrass.form, data = seagrass)
...
seagrass.brm <- brm(seagrass.form, prior = priors, data = seagrass, 
                 sample_prior = 'only', 
                 iter = 5000, 
                 warmup = 1000, 
                 chains = 3, cores = 3, 
...
Check priors:
```{r}
seagrass.brm |> conditional_effects() |> plot(points = TRUE, ask = FALSE)
```


Add the data:
```{r}
seagrass.brm6 <- update(seagrass.brm, sample_prior = 'yes')
```
